...
**Q: 忘记密码怎么办？** 在部署机（服务器/本机）上执行重置命令即可，不影响监控、密钥等其他数据：

```bash
# 本机 python 方式运行
python3 AppPriceTracker.py resetpw admin 新密码至少6位

# Docker 方式运行（容器名默认 app-price）
docker exec app-price python AppPriceTracker.py resetpw admin 新密码至少6位
```

普通用户的密码也可由管理员在「用户」面板重置；admin 自己忘记密码只能用上面的命令行方式。
